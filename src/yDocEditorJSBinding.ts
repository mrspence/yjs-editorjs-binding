...
export class YDocEditorJSBinding {
  ...
  async onBlockEventEditorJS(api: API, event: CustomEvent) {
    ...
    const uuid = this.internalStoreAsArray.find(entry => entry.index === index)?.editorBlock.uuid || genUUID() // get or generate a UUID
  ...
  private initialRenderYdocToEdittorJS() {
    ...
    this.ydocArray.toArray().forEach((editorBlock, index) => this.renderBlock(editorBlock, index))
  }
              ...
              switch(instruction) {
                ...
                case "insert":
                  ...
                  for (const editorBlock of value) {
                    this.internalStore.set(editorBlock.uuid, {index, editorBlock})
                ...
                case "delete":
                  ...
                  for (let i = index; i < index + value; i++) {
                    ...
                    const uuid = editorBlock?.holder.getAttribute("data-y2-uuid") || null
